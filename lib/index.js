...
(function() {
	...
	*/
	Topical.prototype.add = function( topic, options ) {
		...
		this.emit( 'added', topic );
		return this;
	}; // end METHOD add()

	/**
	* METHOD: remove( topic )
	*	Removes a topic and all associated listeners.
	*
	...
	*/
	Topical.prototype.remove = function( topic ) {
		var topics = this._topics;
		if ( typeof topic !== 'string' ) {
			throw new TypeError( 'remove()::invalid input argument. Topic must be a string.' );
		}
		if ( !topics.hasOwnProperty( topic ) ) {
			return this;
		}
		delete( topics[ topic ] );
		this.emit( 'removed', topic );
		return this;
	}; // end METHOD remove()
	...
	*/
	Topical.prototype.subscribe = function( topic, clbk ) {
		var topics = this._topics,
			...
			idx;
		if ( typeof topic !== 'string' ) {
			throw new TypeError( 'subscribe()::invalid input argument. Topic must be a string.' );
		}
		if ( typeof clbk !== 'function' ) {
		...
		if ( !topics[ topic ].options.duplicates ) {
			// Ensure that a callback is only called once per topic emission...
		...
		if ( topics[ topic ].options.max === funcs.length ) {
			this.emit( 'error', {
				'topic': topic,
				'message': 'Maximum number of subscribers exceeded.'
			});
			return this;
		}
		funcs.push( clbk );
	...
	*
	* @param {String} topic - topic from which to unsubscribe
	* @param {Function} clbk - callback to unsubscribe
	* @returns {Topical} Topical instance
	*/
	Topical.prototype.unsubscribe = function( topic, clbk ) {
		var topics = this._topics,
			...
			funcs;
		if ( typeof topic !== 'string' ) {
			throw new TypeError( 'unsubscribe()::invalid input argument. Topic must be a string.' );
		}
		if ( typeof clbk !== 'function' ) {
	...
	*/
	Topical.prototype.publish = function( topic, event ) {
		...
		funcs = topics[ topic ].subscribers;
		for ( var i = 0; i < funcs.length; i++ ) {
			funcs[ i ].apply( this, event );
		}
		if ( funcs.length ) {
			this.emit( 'published', topic );
		}
		return this;
	}; // end METHOD publish()
